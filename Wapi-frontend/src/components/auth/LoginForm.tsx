    ...
    <div className="min-h-screen bg-linear-to-br from-slate-100 via-white-50 to-emerald-50 flex items-center dark:bg-(--page-body-bg) justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-6xl">
        <div className="grid lg:grid-cols-2 gap-0 items-center">
          ...
          <div className="bg-white dark:bg-(--card-color) lg:rounded-br-2xl rounded-tr-2xl shadow-2xl p-4 sm:p-12 min-h-175 flex flex-col justify-center relative">
            ...
            <div className="relative z-10 max-w-md mx-auto w-full">
              ...
              <div className="mb-8">
                <h2 className="text-3xl text-slate-900 font-bold mb-2">{isAuthSetupLoading ? <Skeleton className="h-9 w-48" /> : content.title}</h2>
                <div className="text-slate-500">{isAuthSetupLoading ? <Skeleton className="h-5 w-64" /> : content.subtitle}</div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-3 flex flex-col">
                  ...
                  <div className="grid grid-cols-2 gap-3 p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl relative">
                    <Button type="button" onClick={() => setRoleType("user")} className={`relative z-10! flex items-center justify-center gap-2! py-2.5! rounded-lg! text-sm! font-medium! transition-all duration-300 ${roleType === "user" ? "text-emerald-700! dark:text-emerald-400! bg-white! dark:bg-slate-700! shadow-sm!" : "text-slate-500! bg-[unset]! hover:text-slate-700!"}`}>
                      <User className={`w-4 h-4 transition-transform duration-300 ${roleType === "user" ? "scale-110" : "scale-100"}`} />
                      User
                    </Button>
                    <Button type="button" onClick={() => setRoleType("agent")} className={`relative z-10! flex items-center justify-center gap-2! py-2.5! rounded-lg! text-sm! font-medium! transition-all duration-300 ${roleType === "agent" ? "  bg-white! text-violet-700! dark:text-violet-400! shadow-sm!" : "text-slate-500! bg-[unset]! hover:text-slate-700!"}`}>
